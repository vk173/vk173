...
var colors = ['#000000','#808080', '#c0c0c0', '#ffffff', '#800000', '#ff0000', '#808000', '#ffff00', '#008000', '#00ff00', '#008080', '#00ffff', '#000080', '#0000ff', '#800080', '#ff00ff']
var closeOffset

function swatchClick(){
  chosenColor = f(this).data('color')
  let cursorColor = chosenColor.replace('#', '%23')
  console.log(cursorColor);
  f("#paint").css("cursor",`url('data:image/svg+xml;utf8,<svg width="54" height="54" xmlns="http://www.w3.org/2000/svg" xmlns:svg="http://www.w3.org/2000/svg"><path stroke="black" stroke-width="4" d="m2,27a25,25 0 1 0 25,-25l-25,0l0,25z" fill="`+cursorColor+`" /></svg>'), auto`)
  gsap.to(colorHolder, fillSpeed, { backgroundColor:chosenColor })
  console.log(chosenColor);
}


function colorMe() {
  gsap.to(this, fillSpeed, { fill: chosenColor });
}
function colorRollover(e){
  var rollover = (e.type == 'mouseenter')? {scale:1.2}:{scale:1};
  gsap.to(f(this), 0.05, rollover);
}
function svgRandom() {
  f(svgColor).each(function(){
    var randomNum = Math.floor((Math.random() * colors.length) + 1);
    gsap.to(this, fillSpeed, { fill: colors[randomNum] });
  })
}
function svgClear() {
  f(svgColor).each(function(){
    gsap.to(this, fillSpeed, { fill: "#FFF" });
  })
}
f.fn.makeSwatches = function() {
  var swatchHolder = f('<ul/>', {'class': 'palette'}).appendTo(this)
  f.each(colors, function() {
    var swatch = f('<li/>').appendTo(swatchHolder)
    f(swatch).css('background-color', this)
    f(swatch).data('color', this)
    f(swatch).on('click', swatchClick)
    f(swatch).on('mouseenter mouseleave', colorRollover)
  })
}
f.fn.makeSVGcolor = function(svgURL) {
  mainHolder = this
  f( this ).load(svgURL, function() {
    svgObject  = f('svg', this)
    svgColor   = f('g:nth-child(2)', svgObject).children()
    svgOutline = f('g:nth-child(1)', svgObject).children()
    f(svgColor).on('click', colorMe)
    f('#tools').makeSwatches()
  });
}

f.fn.btnRandom    = function() {
  btnRandom = this
  f(btnRandom).on('click', svgRandom)
}
f.fn.btnClear     = function() {
  btnClear = this
  f(btnClear).on('click', svgClear)
}
}( jQuery ));

jQuery('#paint'   ).makeSVGcolor(svg)
jQuery('#random'     ).btnRandom()
jQuery('#clear'      ).btnClear()
jQuery('#downloadSVG').btnDownload()
      };
